<template>
  <div class="top-banner clearfix" @mouseleave="evtmouseLeave">
    <div class="container">
      <div class="top-side-left">
        <ul class="sideitem">
          <li v-for="item in sideItems" @mouseenter="evtmouseEnter(item.type)">
            {{item.content}}<i class="icon-font fa fa-chevron-right fa-1x fa-fx"></i>
          </li>
        </ul>
      </div>
    </div>
    <div class="menu" v-show="slideStatus">
      <ul class="menu-nav" v-for="items in filtercurrentgoods">
